...
## 修改binguo用户权限只能看到自己的桶

创建自定义策略文件 binguo-policy.json
...
```
mc admin policy detach local-admin readwrite --user binguo
mc admin policy attach local-admin binguo-policy --user binguo
```

验证绑定

```
mc admin policy info local-admin binguo-policy
```
